...
| `pashov-auditor-v4.md` + `pashov-skills-v4/solidity-auditor/` | <https://github.com/pashov/skills> (second sparse clone, **SHA-pinned to V4**, `PASHOV_V4_SHA`) | **STAGED — `two-phase-audit-v3.md` only.** Pashov solidity-auditor V4 (2026-09-23): loop mode + scan memory (both kept OFF by our overrides), agents READ-ONLY in the target, report assembled by `references/assemble.sh` (reviewed: local awk/sed only, no network). Not drop-in: its Turn 1b pass-count question waits for a human. Promote after a rehearsal by moving `PASHOV_SHA` and porting v3's Turn 2 overrides into v2. |
